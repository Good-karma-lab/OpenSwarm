...
- `Darwin arm64` → `wws-connector-0.7.1-macos-arm64.tar.gz`
- `Darwin x86_64` → `wws-connector-0.7.1-macos-amd64.tar.gz`
- `Linux x86_64` or `Linux amd64` → `wws-connector-0.7.1-linux-amd64.tar.gz`
- `Linux aarch64` or `Linux arm64` → `wws-connector-0.7.1-linux-arm64.tar.gz`
- Windows (PowerShell: `$env:PROCESSOR_ARCHITECTURE`) → `wws-connector-0.7.1-windows-amd64.zip`

```bash
ASSET="wws-connector-0.7.1-macos-arm64.tar.gz"  # replace with detected asset
mkdir -p ~/wws-demo && cd ~/wws-demo
curl -L -o connector.tar.gz \
  "https://github.com/Good-karma-lab/World-Wide-Swarm-Protocol/releases/download/v0.7.1/$ASSET"
...
Invoke-WebRequest -Uri "https://github.com/Good-karma-lab/World-Wide-Swarm-Protocol/releases/download/v0.7.1/wws-connector-0.7.1-windows-amd64.zip" -OutFile connector.zip
